 ...
 */
static int
run_openssl_handshake (struct MHD_Connection *connection)
{
	int ret;
 ...
 */
static int
MHD_openssl_connection_handle_read (struct MHD_Connection *connection)
{
  if (MHD_YES == run_openssl_handshake (connection))
    return MHD_YES;
 ...
 */
static int
MHD_openssl_connection_handle_write (struct MHD_Connection *connection)
{
  if (MHD_YES == run_openssl_handshake (connection))
    return MHD_YES;
 ...
 */
static int
MHD_openssl_connection_handle_idle (struct MHD_Connection *connection)
{
  ...
  switch (connection->state){
    ...
    case MHD_CONNECTION_CLOSED:{
      MHD_socket sock;
      sock=SSL_get_fd(connection->ssl);
    ...
    default:
      if ( (SSL_pending(connection->ssl)>0) &&
	   (MHD_YES != MHD_openssl_connection_handle_read (connection)) )
	return MHD_YES;
      return MHD_connection_handle_idle (connection);
    }
#if EPOLL_SUPPORT
 ...
 */
void
MHD_set_https_openssl_callbacks (struct MHD_Connection *connection)
{
  connection->read_handler = &MHD_openssl_connection_handle_read;
  connection->write_handler = &MHD_openssl_connection_handle_write;
  connection->idle_handler = &MHD_openssl_connection_handle_idle;
}
