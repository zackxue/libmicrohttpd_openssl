...
//OPENSSL_SUPPORT
#if HTTPS_OPENSSL_SUPPORT
if (0 != (daemon->options & MHD_OPENSSL))
    {
      connection->recv_cls = &recv_openssl_adapter;
      connection->send_cls = &send_openssl_adapter;
      connection->state = MHD_TLS_CONNECTION_INIT;
      MHD_set_https_openssl_callbacks (connection);
	  if(connection->ssl==0){
		connection->ssl = SSL_new(daemon->ctx);
...
//OPENSSL_SUPPORT
#if HTTPS_OPENSSL_SUPPORT
		case MHD_OPTION_OPENSSL_KEY:
		  daemon->keypath = va_arg (ap, const char *);
		  break;
		case MHD_OPTION_OPENSSL_CERT:
		  daemon->certpath = va_arg (ap, const char *);
		  break;
#endif
//OPENSSL_SUPPORT_END
...
//OPENSSL_SUPPORT
#if HTTPS_OPENSSL_SUPPORT
  if ((0 != (flags & MHD_OPENSSL)) && (0 != MHD_openssl_init (daemon))){
	printf("fault init openssl\n");
	goto free_and_fail;
  }
#endif
//OPENSSL_SUPPORT_END
